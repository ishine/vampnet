...
class Interface(torch.nn.Module):
    ...
    def coarse_to_fine(self, coarse_z: torch.Tensor, **kwargs):
        ...
        if n_codebooks_to_append > 0:
            coarse_z = torch.cat(
                [
                    coarse_z,
                    torch.zeros(
                        coarse_z.shape[0], n_codebooks_to_append, coarse_z.shape[-1]
                    )
                    .long()
                    .to(self.device),
                ],
...
if __name__ == "__main__":
    ...
    mask = mask_and(mask, periodic_mask(z, 32, 1, random_roll=True))
